...
- **Daemon log** (`internal/daemonlog`) — a per-day append-mode file at `%LOCALAPPDATA%\Prata\logs\prata-YYYY-MM-DD.log`. The production binary runs `-H windowsgui` with no console, so the daemon's stderr diagnostics are silently discarded; this file is the only durable record of what happened per dictation. Each per-dictation event (capture start, drops, transcribe/inject outcomes) is logged with the active backend ID and elapsed time. **Metadata only — never the transcribed text** (backend, timings, char counts, error strings), so the log carries no patient content. Best-effort and stdlib-only: a log that cannot be opened or written degrades to stderr and never disrupts dictation. `PRATA_DAEMON_LOG` overrides the path for tests.
