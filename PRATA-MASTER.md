...
4. Normalize the response into running prose and apply dictionary corrections. The segment join is backend-specific (`normalizeTranscript`): the local GPU servers (`Hemma`/`Jobb`, `TrimmedSegments`=false) leave segment text untrimmed and concatenate lines **without** a separator, so long compound words are not split apart ("Tydlighet", not "Tyd lighet"); Berget (`TrimmedSegments`=true) pre-trims each segment, so its line break is joined with a **space** instead, or sentences would glue together
...
- **Hotkey** — global F1 (PTT) and F8 (dictionary quick-fix) via `RegisterHotKey`. If another program already owns F1, the daemon does **not** exit: it stays alive, cues + balloons + shows a persistent `F1 UPPTAGEN` tray badge, and re-probes every 3 s, re-registering the instant F1 frees (e.g. the conflicting program closes) — self-healing, no restart. F8 registers independently of F1.
...
- **Backend selector** — Rngv GPU-server (Tailscale) / LAN GPU-server / Berget Ai as radio buttons in the tray menu; the active backend is shown in the tooltip + balloon. The choice is saved as a stable ID (`Hemma` / `Jobb` / `Berget`) in `%LOCALAPPDATA%\Prata\backend.txt` — the display names can change without breaking saved choices. **Default on first start (missing or invalid `backend.txt`): LAN GPU-server (`Jobb`)** — internal GPU without an API key. Conditional auth (only Berget Ai sends Bearer). No silent failover — but a local **keyless** backend that fails to respond on two consecutive dictations raises a one-time tray balloon suggesting a manual switch (e.g. to Berget Ai); the streak resets on any backend response (even empty/degenerate text), so each separate outage hints exactly once; nothing switches on its own and patient audio is never auto-routed to the cloud (`internal/failover`). While such an outage streak is active the tray tooltip also carries a **persistent `SVARAR INTE` suffix** (`tray.SetDegraded`) — a non-fading health surface that outlasts the one-time balloon and clears on the next successful response. See `PRATA-GPU-SERVER.md`.
- **Dictionary** — two layers: (1) a **baseline** embedded in the binary at build time (`go:embed` of `internal/dict/dictionary-corrections.txt`); (2) a **per-user override** in `%LOCALAPPDATA%\Prata\dictionary-corrections.txt` (F8 writes here). The override is layered on top of the baseline (replacing per key). Unicode-aware word-boundary replacements (literal `strings.Index`, no regexp). Valuable override entries can be folded into the baseline before a release with the `cmd/dict-foldin` build-time tool (developer-run, never in CI or the daemon).
- **Text injection** — class-based routing: Chromium/Electron (class `Chrome_WidgetWin_1`, including the web-based medical record) and Notepad++ (class `Notepad++`, whose Scintilla editor reads the clipboard slower than the old 50 ms paste-settle delay allowed, so the restore's `EmptyClipboard` wiped the dictated text before it landed — fixed by raising `pasteSettleDelay` to 400 ms for all clipboard targets, with Notepad++ additionally routed to SendInput as race-immune) → `SendInput` Unicode, the whole string in a single call, the clipboard is never touched; other windows (e.g. Word, classic Notepad) → clipboard paste (`CF_UNICODETEXT`, save/restore; every Prata clipboard write — the dictated text and the restore of the user's prior clipboard alike — is marked to stay out of clipboard history (Win+V), the cloud clipboard, and clipboard monitors, so Prata never adds an entry to the user's Win+V). See Decision 6.
- **Daemon log** (`internal/daemonlog`) — a per-day append-mode file at `%LOCALAPPDATA%\Prata\logs\prata-YYYY-MM-DD.log`. The production binary runs `-H windowsgui` with no console, so the daemon's stderr diagnostics are silently discarded; this file is the only durable record of what happened per dictation. Each per-dictation event (capture start, drops, transcribe/inject outcomes) is logged with the active backend ID and elapsed time. **Metadata only — never the transcribed text** (backend, timings, char counts, error strings), so the log carries no patient content. Best-effort and stdlib-only: a log that cannot be opened or written degrades to stderr and never disrupts dictation. `PRATA_DAEMON_LOG` overrides the path for tests. Logs older than 30 days are pruned on startup so the directory stays bounded over years of unattended running.
...
|---|---|---|---|
| **`prata.exe --install`** (double-click `Installera-Prata.bat`) | `%ProgramFiles%\Prata\prata.exe` | Machine-wide Task Scheduler (`Prata`, all users, RunLevel Limited, restart-on-failure 3× / 1 min) | Implemented (install/uninstall/update, Phases 5–7) |
...
- **Writable state** always lives per user in `%LOCALAPPDATA%\Prata\` (`apikey.dat`, `backend.txt`, dictionary override, and `logs\prata-YYYY-MM-DD.log` — see "Daemon log" below). No machine-wide writable data in `%ProgramData%`.
