...
- **Text injection** — class-based routing: Chromium/Electron (class `Chrome_WidgetWin_1`, including the web-based medical record) → `SendInput` Unicode, the whole string in a single call, the clipboard is never touched; other windows → clipboard paste (`CF_UNICODETEXT`, save/restore; the dictated text is marked to stay out of clipboard history (Win+V), the cloud clipboard, and clipboard monitors, while the restore of the user's prior clipboard is left unmarked). See Decision 6.
