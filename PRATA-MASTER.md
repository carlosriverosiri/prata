...
- **Text injection** — class-based routing: Chromium/Electron (class `Chrome_WidgetWin_1`, including the web-based medical record) and Notepad++ (class `Notepad++`, whose Scintilla editor rejects the clipboard markers) → `SendInput` Unicode, the whole string in a single call, the clipboard is never touched; other windows (e.g. Word, classic Notepad) → clipboard paste (`CF_UNICODETEXT`, save/restore; every Prata clipboard write — the dictated text and the restore of the user's prior clipboard alike — is marked to stay out of clipboard history (Win+V), the cloud clipboard, and clipboard monitors, so Prata never adds an entry to the user's Win+V). See Decision 6.
